...
public class Bank : BuildingEffect
{
    ...
    // Ez a metódus végigmegy az összes szomszédos mezõn, és ha Houses-t talál, bónuszt ad
    private void CheckSurroundingTiles()
    {
        // Iterálunk a 3x3-as területen, ahol a Houses található
        ...
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                ...
                if (checkX >= 0 && checkX < tilesBuilder.Tiles.GetLength(0) && checkY >= 0 && checkY < tilesBuilder.Tiles.GetLength(1))
                {
                    ...
                    {
                        // Bónuszt adunk, ha Houses-t találunk
                        Debug.Log($"Kezdeti Houses-t talált a {checkX},{checkY} pozíción!");
    ...
    protected override void CheckNewTile(int x, int y, int tileType)
    {
        ...
        {
            Debug.Log($"Új Houses a {x},{y} pozíción, bónuszt kapsz!");
            Income.Instance.gold_income += 1;
        }
    }
}
