...
public class Fishery : BuildingEffect
{
    ...
    // Ez a metódus végigmegy az összes szomszédos mezõn, és ha Fish-t talál, bónuszt ad
    private void CheckSurroundingTiles()
    {
        // Iterálunk a 3x3-as területen, ahol a Fish található
        ...
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                ...
                if (checkX >= 0 && checkX < tilesBuilder.Tiles.GetLength(0) && checkY >= 0 && checkY < tilesBuilder.Tiles.GetLength(1))
                {
                    ...
                    {
                        // Bónuszt adunk, ha Fish-t találunk
                        Debug.Log($"Kezdeti Fisht-t talált a {checkX},{checkY} pozíción!");
    ...
    protected override void CheckNewTile(int x, int y, int tileType)
    {
        ...
        {
            Debug.Log($"Új Fish a {x},{y} pozíción, bónuszt kapsz!");
            Income.Instance.food_income += 1;
        }
    }
}
